...
--     vehiculos_a_insertar INTEGER := 0;
--     id_compra_actual INTEGER;
--     ya_conteo_existente BOOLEAN := FALSE;
-- BEGIN
--     IF NEW.incluye_parking THEN
--         -- Obtener límite de vehículos
...
--         -- Total de vehículos ya reservados
--         SELECT COALESCE(SUM(c.cant_vehiculos), 0)
...
--         -- Obtener id_compra de este pase
--         SELECT c.id_compra INTO id_compra_actual
--         FROM compras c
--         JOIN pases p ON c.id_compra = p.id_compra
--         WHERE p.id_pase = NEW.id_pase;

--         -- Verificar si ya se contó esta compra para esta fecha y parque
--         SELECT EXISTS (
--             SELECT 1
--             FROM pases p2
--             JOIN pases_parques pp2 ON p2.id_pase = pp2.id_pase
--             WHERE p2.id_compra = id_compra_actual
--               AND pp2.fecha_acceso = NEW.fecha_acceso
--               AND pp2.id_parque = NEW.id_parque
--               AND pp2.incluye_parking = TRUE
--         ) INTO ya_conteo_existente;

--         -- Solo sumar si no fue contada antes
--         IF NOT ya_conteo_existente THEN
--             SELECT cant_vehiculos INTO vehiculos_a_insertar
--             FROM compras
